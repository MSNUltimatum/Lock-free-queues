...
int lstack_init(lfstack_t *lstack, size_t max_size)
{
    struct lfs_head head_init = {0, NULL};
    lstack->head = ATOMIC_VAR_INIT(head_init);
    lstack->size = ATOMIC_VAR_INIT(0);
...
struct lfs_node *pop(_Atomic struct lfs_head *head) {
...
static void push(_Atomic struct lfs_head *head, struct lfs_node *node) {
    struct lfs_head orig = atomic_load(head);
    struct lfs_head next;
    do {
        node->next = orig.node;
